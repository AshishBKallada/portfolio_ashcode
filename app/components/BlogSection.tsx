...
export default function BlogSection() {
  return (
    <div
      id="blog"
      className="flex w-full shrink-0 flex-col justify-center bg-transparent px-6 py-12 text-white md:px-12 md:py-16 lg:px-16"
    >
      <div className="mx-auto flex w-full max-w-[1600px] flex-col gap-6 md:gap-8">
        {/* Heading */}
        <div className="relative inline-block max-w-full">
          <div className="flex flex-wrap items-start gap-3 sm:items-center">
            <h1 className="font-chaney text-5xl font-bold leading-none text-white md:text-6xl lg:text-7xl">
              Blog
        ...
        <div className="flex max-w-2xl flex-col gap-2">
          <h2 className="border-b border-black/20 pb-2 font-safiro text-xs font-semibold uppercase tracking-wider text-white md:pb-3 md:text-sm">
            ABOUT
          </h2>
          <p className="text-sm leading-snug text-white/80 md:text-base md:leading-relaxed">
        ...
        <div className="flex w-full flex-col border-t border-black/15 pt-5 md:pt-6">
          <div className="mb-0 grid grid-cols-[minmax(0,7rem)_1fr] gap-4 border-b border-black/20 pb-2 sm:grid-cols-[140px_1fr] md:grid-cols-[160px_1fr] md:gap-6 md:pb-3">
            <div className="font-safiro text-xs font-semibold uppercase tracking-wider text-white/60 md:text-sm">
              DATE
            </div>
            <div className="font-safiro text-xs font-semibold uppercase tracking-wider text-white/60 md:text-sm">
              NAME
          ...
          <div className="flex flex-col">
            {blogPosts.map((post, index) => (
              ...
              >
                <div className="text-sm text-white transition-colors duration-200 group-hover:text-white group-focus-visible:text-white md:text-base">
                  {post.date}
                </div>
                <div className="flex items-center justify-between gap-3 text-sm text-white transition-colors duration-200 group-hover:text-white group-focus-visible:text-white md:text-base">
                  <span className="min-w-0 leading-snug">{post.name}</span>
                  <span className="shrink-0 text-lg font-bold text-white/60 transition-colors duration-200 group-hover:text-white group-focus-visible:text-white">
                    +
                  </span>
                </div>
              </Link>
